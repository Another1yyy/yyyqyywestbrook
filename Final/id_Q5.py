# -*- coding: utf-8 -*-
"""
Per-Bomb Interference Reporter (Point Target)
--------------------------------------------
功能:
...
- 导弹: 速率 300 m/s, 直线飞向原点 O=(0,0,0) (题面设定)
- 遮蔽判据: 距离(云团中心, 线段[ M(t) -> T ]) <= 10 m

注:
- 这里专注“每枚烟幕弹对各导弹的遮蔽效果”，未做“三弹同时交集”
- 若需要交集(如Q5验证器)，可复用 per-missile union 后取交
...
def explosion_point(F0: np.ndarray, v: float, theta_rad: float, t_drop: float, tau: float):
    """无人机等高匀速直线: 先飞行 t_drop 再脱离; 脱离后经历 tau 再起爆"""
      ...
      results: 列表, 每个元素是一枚烟幕弹的报告:
        {
          ...
          'interfered_missiles': ['M1','M3']  # 时长>0者
        }
    """
    # 归一化导弹
    missiles_norm = []
    ...
    for uav in uav_list:
        ...
        for k, (td, ta) in enumerate(zip(t_drop, tau), start=1):
            ...
            if skip_ground_explosion and E[2] <= 0.0:
                if verbose:
                    print(f"[WARN] {name}#{k} 起爆高度 Ez={E[2]:.3f}<=0, 跳过。")
                continue
            ...
            if verbose:
                print(f"\n[{name}#{k}]  E=({E[0]:.3f},{E[1]:.3f},{E[2]:.3f}), t_e={t_e:.3f}s")
                for m_name in per_m:
