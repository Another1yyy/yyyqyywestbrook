...
M0      = np.array([19000.0, 600, 2100.0], dtype=float)  # 导弹初始
u_m     = (np.array([0.0,0.0,0.0]) - M0) / np.linalg.norm(M0)
T_HIT   = np.linalg.norm(M0) / vm

FY1_F0  = np.array([12000, 1400, 1400.0])  # FY1 初始（等高直线）
...
def main():
    ...
    pso = PSO(dim, bounds, pop_size=500, iters=220,
              w_start=0.9, w_end=0.3, c1=1.7, c2=1.7,
              v_max_scale=0.5)
