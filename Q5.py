...
def solve_problem4_collaborative(missile_trajectories):
    ...
    def save_callback(xk, convergence):
        history.append((xk.copy(), convergence))
    result = differential_evolution(
        ...
        maxiter=300, popsize=30, disp=True, workers=-1,updating='deferred', polish=True,callback=save_callback
    )
    prev_gen_best = history[-2][0]
    prev_gen_best = np.asarray(prev_gen_best)
    ...
    if result.success and result.fun < 1e8:
        ...
        all_points = prev_gen_best.reshape(len(drone_names) * bombs_per_drone, 4)
...
def solve_problem5_collaborative(missile_trajectories):
    ...
    if result.success and result.fun < 1e8:
        ...
        all_points = prev_gen_best.x.reshape(len(drone_names) * bombs_per_drone, 4)
        results_dict = {name: {'strategies': []} for name in drone_names}
        for i, point in enumerate(all_points):
            drone_idx = i // bombs_per_drone
            drone_name = drone_names[drone_idx]
