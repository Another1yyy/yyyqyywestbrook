...
class DroneObscurationOptimizer:
    ...
    def _get_coverage_from_params(self, params):
        ...
        coverages = [simulate_coverage(*params[i * 4:(i + 1) * 4], F0=self.drones[name])[0] for i, name in
                     enumerate(self.drone_names)]
    ...
    def solve_de(self, pop_size=50, max_iter=100, seed=None):
        ...
        result = differential_evolution(self.de_objective, bounds, popsize=pop_size, maxiter=max_iter,
                                        workers=self.processes, updating='deferred',
                                        callback=lambda xk, conv: pbar.update(1), init=initial_population, seed=seed)
        pbar.close()
    ...
    def single_drone_bo(self, name, fixed_params=None, n_calls=50, n_initial_points=10):
        ...
        res = gp_minimize(objective, space, n_calls=n_calls, n_initial_points=n_initial_points, random_state=42,
                          callback=lambda res: pbar_bo.update(1))
        pbar_bo.close()
    ...
    def run_optimization_pipeline(self, de_pop_size, de_max_iter, bo_calls, bo_initial_points, n_random_seeds,
                                  seed_file_path):
        ...
        if zero_drones and self.best_coverage > 0:
            ...
            # 使用字典推导式优化重复部分
            fixed_params = {name: self.best_params[i * 4:(i + 1) * 4].tolist() for i, name in
                            enumerate(self.drone_names) if name not in zero_drones}
...
if __name__ == "__main__":
    ...
    optimizer.run_optimization_pipeline(DE_POP_SIZE, DE_MAX_ITER, BO_CALLS, BO_INITIAL_POINTS, N_RANDOM_SEEDS,
                                        SEED_FILE_PATH)
