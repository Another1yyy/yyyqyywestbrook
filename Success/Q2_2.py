...
def sample_cylinder_points(base, radius, height,
    ...
    for th in thetas:
        ...
        for z in zs:
            points.append([base[0] + radius * c, base[1] + radius * s, base[2] + z])
    for z in [0.0, height]:
        ...
        for r in rs:
            for th in thetas_d:
                c, s = np.cos(th), np.sin(th)
                points.append([base[0] + r * c, base[1] + r * s, base[2] + z])
...
def cloud_center_builder(E, t_e):
    return lambda t: E + np.array([0.0, 0.0, -sink_v]) * (t - t_e)
